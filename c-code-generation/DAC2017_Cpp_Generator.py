...
if __name__ == "__main__":
    ...
    for row in csv_reader:
       ...
       # layer_type = ZeroPadding2D(I,O)
        ...
        # layer_type = Dense(I,W,B,O) (Activation option : relu , softmax)
            ...
            Output_variables += "static DATA_T O"+line_num_str+ "_SW[" + output_shape[1] + "];\n"
            #Static_variables to Cpp.txt
            Static_variables += "static DATA_T W"+line_num_str+"_d["+ output_shape[1] + "][" + input_shape[1] + "];\n";
            Static_variables += "static DATA_T B"+line_num_str + "_d[" + output_shape[1] + "];\n";
	    #variables to top.txt
            variables += "DATA_T W"+line_num_str+"["+ output_shape[1] + "][" + input_shape[1] + "], "
            variables += "DATA_T B"+line_num_str + "[" + output_shape[1] + "],"
            #variables to top_func.txt
            variables_i += "DATA_T W"+line_num_str+"_d["+ output_shape[1] + "][" + input_shape[1] + "], "
            variables_i += "DATA_T B"+line_num_str + "_d[" + output_shape[1] + "],"
            #assign_value to top.txt
            assign_value += "B"+line_num_str+"_d_m_loop: for (m=0; m<"+output_shape[1]+"; m++) {\n"+"\tB"+line_num_str+"_d[m] = B"+line_num_str+"[m];\n}\n"
            assign_value += "W"+line_num_str+"_d_m_loop: for (m=0; m<"+output_shape[1]+"; m++) {\n  W"+line_num_str+"_d_k_loop: for (k=0; k<"+input_shape[1]+"; k++) {\n  W"+line_num_str+"_d[m][k] = W"+line_num_str+"[m][k];\n  }\n}\n"
        	#function def
            ...
            if line_count<=1 :
                HW_functions += "HW_" + row["name"]+"(I_strm, W"+line_num_str+"_d, B"+line_num_str+"_d, O"+line_num_str+"_strm);\n"
            else :
                HW_functions += "HW_" + row["name"]+"(O"+str(line_count-1)+"_strm, W"+line_num_str+"_d, B"+line_num_str+"_d, O"+line_num_str+"_strm);\n"
            Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_d complete dim=1\n"
            Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_d complete dim=3\n"
            Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_d complete dim=4\n"
            Optimized_code += "#pragma HLS ARRAY_PARTITION variable=B"+line_num_str+"_d complete\n"
	...
	#Template top_func.txt
        tf={'model_name':"DAC2017_"+model_name,'variables': variables_i,
        ...
        #Template top.txt
        to={'model_name':"DAC2017_"+model_name, 'variables':variables, 'Output_channel':output_shape[3],'Output_width':output_shape[1],
    ...
    # Output shape 1D
        ...
        variables_i += "DATA_T O["+output_shape[1]+"]"
 	#Template Stream_io.txt
        ...
        #Template top_func.txt
        tf={'model_name':"DAC2017_"+model_name,'variables': variables_i, 'Optimized_code':Optimized_code ,'Stream_declaration':Stream_declaration ,
	'Function_call': HW_functions, 'line_num':line_num_str}
        top_func_template=top_func.substitute(tf)
        #Template top.txt
        to={'model_name':"DAC2017_"+model_name, 'variables':variables, 'last_output_channel':output_shape[1],'assign_value':assign_value,
