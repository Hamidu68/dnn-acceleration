...
if __name__ == "__main__":
    ...
    i_func_argument="" #Argument to top(Our).txt
    DAC_func_argument="" #Argument to DAC.txt
    Output_variables="" #Argument to sw.txt
    i_assign_value="" #Argument to top(Our).txt
    DAC_assign_value = "" #Argument to DAC.txt
    i_Optimized_code="" #Argument to top_func.txt
    DAC_Optimized_code =""
    variables="" #Argument to top.txt
    variables_i = "" #Argument to top(Our).txt
    ...
    for row in csv_reader:
        ...
        if row["layer_type"] == 'Conv2D' :
            ...
            variables_i += "DATA_T W"+line_num_str+"_i["+ output_shape[3] + "][" + input_shape[3] + "][" + filter_shape[0] + "][" + filter_shape[1] + "], "
            variables_i += "DATA_T B"+line_num_str + "_i[" + output_shape[3] + "],";
            ...
            #Function definition
            i_def_func += Conv2D_same_hw.substitute(lm) +"\n"
            DAC_def_func += Conv2D_same_DAC2017.substitute(lm) +"\n"
            #Function use
            if line_count<=1 :
                i_functions += "HW_" + row["name"]+"(I_strm, W"+line_num_str+"_i, B"+line_num_str+"_i, O"+line_num_str+"_strm);\n"
                ...
                i_functions += "HW_" + row["name"]+"(O"+str(line_count-1)+"_strm, W"+line_num_str+"_i, B"+line_num_str+"_i, O"+line_num_str+"_strm);\n"
            #Optimized code
            i_Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_i complete dim=1\n"
            i_Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_i complete dim=3\n"
            i_Optimized_code += "#pragma HLS ARRAY_PARTITION variable=W"+line_num_str+"_i complete dim=4\n"
            i_Optimized_code += "#pragma HLS ARRAY_PARTITION variable=B"+line_num_str+"_i complete\n"
        ...
        # layer_type = MaxPooling2D (I, O)
            ...
            #Function use
            i_functions += "HW_" + row["name"]+"(O"+str(line_count-1)+"_strm, O"+line_num_str+"_strm);\n"
    ...
    tf={'model_name':model_name,'variables': variables_i,
    'Optimized_code':i_Optimized_code ,'Stream_declaration':Stream_declaration , 'Function_call': i_functions, 'line_num':line_num_str}
    our_func_template=top_func.substitute(tf)
   ...
   #Template for model function.txt
    to={'model_name':model_name, 'variables':variables, 'Output_channel':output_shape[3],'Output_width':output_shape[1],
        'Output_height':output_shape [2], 'assign_value':i_assign_value, 'top_func_argument':i_func_argument}
    our_top_template=top.substitute(to)
    ...
    f = {'D_type':sys.argv[3], 'Stream_io':stream_template, 'Static_variables': Static_variables, 'top_func':our_func_template, 'top':our_top_template , 'DAC2017' : DAC_top_template, 'DAC2017_func': DAC_func_template, 'our_def_func':i_def_func, 'DAC2017_def_func': DAC_def_func}
