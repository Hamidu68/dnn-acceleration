...
if __name__ == "__main__":
    ...
    for row in csv_reader:
       ...
       # layer_type = ZeroPadding2D(I,O)
        ...
        # layer_type = Dense(I,W,B,O) (Activation option : relu , softmax)
            ...
            Output_variables += "static DATA_T O"+line_num_str+ "_SW[" + output_shape[1] + "];\n"
            #Static_variables to Cpp.txt
            Static_variables += "static DATA_T W"+line_num_str+"_DAC["+ output_shape[1] + "][" + input_shape[1] + "];\n";
            Static_variables += "static DATA_T B"+line_num_str + "_DAC[" + output_shape[1] + "];\n";
	    #variables to top.txt
            variables += "DATA_T W"+line_num_str+"["+ output_shape[1] + "][" + input_shape[1] + "], "
            variables += "DATA_T B"+line_num_str + "[" + output_shape[1] + "],"
            #variables to top_func.txt
            variables_dac += "DATA_T W"+line_num_str+"_DAC["+ output_shape[1] + "][" + input_shape[1] + "], "
            variables_dac += "DATA_T B"+line_num_str + "_dac[" + output_shape[1] + "],"
            #assign_value to top.txt
            assign_value += "B"+line_num_str+"_DAC_m_loop: for (m=0; m<"+output_shape[1]+"; m++) {\n"+"\tB"+line_num_str+"_DAC[m] = B"+line_num_str+"[m];\n}\n"
            assign_value += "W"+line_num_str+"_DAC_m_loop: for (m=0; m<"+output_shape[1]+"; m++) {\n  W"+line_num_str+"_DAC_k_loop: for (k=0; k<"+input_shape[1]+"; k++) {\n  W"+line_num_str+"_DAC[m][k] = W"+line_num_str+"[m][k];\n  }\n}\n"
        	#function def
    ...
    # Output shape 1D
        ...
        variables_dac += "DATA_T O["+output_shape[1]+"]"
 	#Template Stream_io.txt
