...
if __name__ == "__main__" :
    ...
    if len(output_shape) == 4 :
        ...
        model_function =  "DAC2017_top" + func_model + ",O_HW);\n"
        model_function += "  " +"DAC2017_sw" + func_model + """,O_SW);\n
   int err_cnt = 0;
   for (m=0; m<"""+output_shape[3]+"""; m++) {
    ...
    for (m=0; m<"""+output_shape[1]+"""; m++) {
        if (O_HW[m] != O_SW[m]) {
            printf("DAC_SW: O[%d] = %d", m, O_SW[m]);
            printf("DAC_HW: O[%d] = %d", m, O_HW[m]);
            err_cnt++;}
    }\n"""

    model_definition = "void DAC2017_top" + def_model
    model_definition += "void DAC2017_sw" + def_model
