...
for (m = 0; m < 1000 ; m++) {
	...
	printf("[c_verifier.cpp]Calculate Flatten22\n");
...
for (k = 0; k <  25088 ; k++) {
	fprintf(o_stream,"%.6f ",(float)O22_SW[k]);
	fprintf(c_num,"%.6f ",(float)O22_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
...
for (k = 0; k <  4096 ; k++) {
	fprintf(o_stream,"%.6f ",(float)O23_SW[k]);
	fprintf(c_num,"%.6f ",(float)O23_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
	...
	fprintf(o_stream,"%s","Dense : [[");
for (k = 0; k <  4096 ; k++) {
	fprintf(o_stream,"%.6f ",(float)O24_SW[k]);
	fprintf(c_num,"%.6f ",(float)O24_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
	...
	fprintf(o_stream,"%s","Dense : [[");
for (k = 0; k <  1000 ; k++) {
	fprintf(o_stream,"%.6f ",(float)O25_SW[k]);
	fprintf(c_num,"%.6f ",(float)O25_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
