...
int main(int argc, char *argv[]){
	...
	static DATA_T W48[200][512];
	static DATA_T B48[200];
...
for (k = 0; k <  512 ; k++) {
	fprintf(o_stream,"%.6f ",(DATA_T)O47_SW[k]);
	fprintf(c_num,"%.6f ",(DATA_T)O47_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
...
for (k = 0; k <  200 ; k++) {
	fprintf(o_stream,"%.6f ",(DATA_T)O48_SW[k]);
	fprintf(c_num,"%.6f ",(DATA_T)O48_SW[k]);
}
fprintf(o_stream,"%s","]]\n\n");
