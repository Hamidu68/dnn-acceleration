...
class ZeroPadding2D(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post=''):
        super(ZeroPadding2D, self).__init__(config, inputs, dtype, layer_odr, post)
...
class ZeroPadding2D_HW(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post=''):
        ...
        # code
        zp = open(self.template_path + "function/HW_zeropadding.txt")
        template = zp.read()
        func = template.format(Name=self.config["name"], Input_channel=input_shape[3], Input_width=input_shape[1],
                               Input_height=input_shape[2], Padding_size=padding[0][0])
        self.function['code'] = func + "\n"