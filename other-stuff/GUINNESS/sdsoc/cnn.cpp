...
// weight memory -----------------------------------------------------------
ap_int<3>  conv0W[64][3*3];
ap_int<64>  conv1W[128][3*3];
...
#pragma SDS data zero_copy(t_bin_convW[0:223296])
#pragma SDS data zero_copy(t_BNFb[0:323])
#endif
void setup(
#ifdef __SDSCC__
	    int *t_bin_convW,
...
)
{
    ...
    for( of = 0; of < 64; of++){
        for( y = 0; y < 3; y++){
            for( x = 0; x < 3; x++){
                ap_uint<3>tmp = 0x1;
                for( inf = 0; inf < 3; inf++){
                     if( t_bin_convW[of*3*3*3+inf*3*3+y*3+x+offset] == 1){
                         conv0W[of][y*3+x] |= tmp;
                     }
                tmp = tmp << 1;
                }
        ...
        for( y = 0; y < 3; y++){
            for( x = 0; x < 3; x++){
                ...
                for( inf = 0; inf < 128; inf++){
                     if( t_bin_convW[of*128*3*3+inf*3*3+y*3+x+offset] == 1){
                         conv2W[of][y*3+x] |= tmp;
                     }
                tmp = tmp << 1;
                }
    ...
    for( of = 0; of < 3; of++){
        for( inf = 0; inf < 128; inf++){
            fc0W[of][inf] = (ap_int<1>)t_bin_convW[of*128+inf+offset];
        }
    }
		...
		)
{
	...
	static ap_int<128> shift_reg1[(48+2)*3];
#pragma HLS ARRAY_PARTITION variable=shift_reg1 complete dim=1
	static ap_uint<1> padding_shift_reg[(48+2)*3];
...
#pragma HLS loop_flatten off
    	...
    	SHIFT_REG: for( i = 0; i < 2 * (48+2) + 3; i++){
#pragma HLS UNROLL
    	...
    	switch( layer){
        case  1:
        shift_reg1[ 2 * (48+2) + 3 - 1] = din;
        padding_shift_reg[ 2 * (48+2) + 3 - 1] = padding; break;
        break;
        case  2:
        shift_reg1[ 2 * (48+2) + 3 - 1] = din;
        padding_shift_reg[ 2 * (48+2) + 3 - 1] = padding; break;
		...
		ap_uint<128> bit_tmp = 0x1;
		ap_uint<128> streamOut = 0;
...
#pragma HLS pipeline
            	CONV_KX: for( kx = 0; kx < 3; kx++){
            		...
            		ap_uint<128> bxor;
                    ap_uint<128> mask;
                    ap_uint<128> allzero = 0;
                    ap_uint<1>is_padding;

            		switch( layer){
                        case 1:
                            bx = shift_reg1[ky * (48+2) + kx];
                            bw = (ap_uint<128>)conv1W[ofeat][ky*3+kx];
                            mask = ~(~allzero << 64);
                            is_padding = padding_shift_reg[ky * (48+2) + kx];
                        break;
                        case 2:
                            bx = shift_reg1[ky * (48+2) + kx];
                            bw = (ap_uint<128>)conv2W[ofeat][ky*3+kx];
                            mask = ~(~allzero << 128);
                            is_padding = padding_shift_reg[ky * (48+2) + kx];
                        break;
                        default: break;
                    ...
                    bxor = (ap_uint<128>)(bx ^ bw) & mask;

			tmp2 = 0;
                    ONES_COUNT: for( i = 0; i < 128; i++){
...
#pragma HLS pipeline
            	...
            	if( cnt == n_out){
            		cnt = 0;
            		fmap[oy][ox] = (ap_int<128>)streamOut;
...
#pragma SDS data zero_copy(t_in_img[0:48*48])
#endif
void kernel(
#ifdef __SDSCC__
...
{
	ap_int<128> fb_tmp[48][48];
	ap_int<1> fc_tmp[128];
	ap_int<64> in_img[48][48];

	int y, x, of, layer, bin_layer_idx;
    int fsize[6] = { 48, 48, 48, 48, 24,  1};
    int n_in[6]  = {  3, 64,128,128,128,128};
    int n_out[6] = { 64,128,128,128,128,  3};


	for( y = 0; y < 48; y++){
		for( x = 0; x < 48; x++){
			in_img[y][x] = t_in_img[y*48+x];
		}
	}
...
#pragma HLS INLINE
    ...
    bin_layer_idx = 1;
	BCONV: for( layer = 0; layer < 6; layer++){
		switch(layer){
            case 0:
            int_conv2d_layer<bit_64, bit_128, 64, 64, 48, 48>
            ...
            break;
            case 3:
            max_pooling_layer<bit_128, 48, 1>(fb_tmp);
            break;
            case 4:
            {
                ap_int<128>mask = 0x1;
            ...
            break;
            case 5:
            fc_layer< 3, 128>( fc_tmp, fc0W, b3_BNFb, fc_result);
            break;
            default: break;
