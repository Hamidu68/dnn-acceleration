...
####################################################
####################################################
#Functions about input layers name & outputs_dict
def get_single_input(row={}, outputs_dict={}):
    return outputs_dict[row['connected_to']]

def get_multi_inputs(row={}, outputs_dict={}):
    return [outputs_dict[name] for name in row['connected_to'].split('/')]

def get_output(line_count):   
    return 'O{}_SW'.format(line_count)  # ex) O1_SW

def get_weight_name(line_count):
    return 'W{}'.format(line_count)     # ex) W1

def get_bias_name(line_count):
    return 'B{}'.format(line_count)     # ex) B1

def get_static_variables(name='', shape=()):
    result = 'static DATA_T {}'.format(name)
    for i in len(shape):
        result += '[{}]'.format(shape[i])
    result += ';\n\t'
    return result           # ex) static DATA_T W1[64][3][3][3];

def get_function_use(name='', param_names=[]):
    result = 'SW_{}('.format(name)
    for i in len(param_names):
        result += '{},'.format(param_names[i])
    result = result[:-1] + ');\n\t'
    return result           # ex) SW_block1_conv1(O1,O2,W2,B2);

####################################################
# layer_type = InputLayer
def add_InputLayer(row=0, line_count=0, templates={}):
    output_name = 'I'
    #output_name = 'I{}'.format(line_count)
    result = ['','','','','','']
    ...
    result[2] = get_static_variables(output_name, input_shape(1:4))
...
def add_Conv2D(row=0, line_count=0, outputs_dict={}, templates={}):
    input_name = get_single_input(row, outputs_dict)
    output_name = get_output(line_count)
    result = ['','','','','','']
    ...
    weight_name = get_weight_name(line_count)
    result[1] = get_static_variables(weight_name, shape=(output_shape[3],input_shape[3],kernel_size[0],kernel_size[1]))
    if eval(row['use_bias']) == True:
        bias_name = get_bias_name(line_count)
        result[1] += get_static_variables(bias_name, shape=(output_shape[3],))
    
    #SW_static_variables(O)
    result[2] = get_static_variables(output_name, output_shape(1:4))
    ...
    #Function use
    param_names = [input_name, output_name, weight_name]
    if eval(row['use_bias']) == True:
        param_names.append(bias_name)
    result[4] = get_function_use(row['name'], param_names)
...
def add_MaxPooling2D(row=0, line_count=0, outputs_dict={}, templates={}):       
    ...
    #Initialization
    #Function use
    result[4] = get_function_use(row['name'], [input_name,output_name])
    #print result
    result[5] = templates['Print']['3D'].substitute({'Name':row['layer_type'],
                                                     'third':output_shape[1],
                                                     'second':output_shape[2],
...
def add_Flatten(row=0, line_count=0, outputs_dict={}, templates={}):
    ...
    #Shared_variables
    #SW_static_variables(O)
    result[2] = get_static_variables(output_name, shape=(output_shape[1],))
    #Initialization
    #Function use
    result[4] = get_function_use(row['name'], [input_name,output_name])
    #print result
...
def add_Dense(row=0, line_count=0, outputs_dict={}, templates={}):       
    ...
    #Shared_variables
    weight_name = get_weight_name(line_count)
    result[1] = get_static_variables(weight_name, shape=(output_shape[1],input_shape[1]))
    if eval(row['use_bias']) == True:
        bias_name = get_bias_name(line_count)
        result[1] += get_static_variables(bias_name, shape=(output_shape[1],))
...
def add_Add(row=0, line_count=0, outputs_dict={}, templates={}):
    input_names = get_multi_inputs(row, outputs_dict)
    output_name = get_output(line_count)
    result = ['','','','','','']
    ...
    #Initialization
    #Function use
    result[4] = get_function_use(row['name'], [input_names[0],input_names[1],output_name])
    #print result
    result[5] = templates['Print']['3D'].substitute({'Name':row['layer_type'],
                                                     'third':output_shape[1],
                                                     'second':output_shape[2],
...
if __name__ == "__main__":
    ...
    for row in csv_reader:
        ...
        if layer_type == 'InputLayer':
            #append output array name of current layer to outputs_dict
            #get result strings
