...
static DATA_T I_i[3][64][64];
	...
	static DATA_T W48_i[200][512];
	static DATA_T B48_i[200];
		...
		HW_activation_1_y_loop: for (y = 0; y < 64; y++) {
			HW_activation_1_m_loop: for (m = 0; m < 64; m++) {
				...
				if (ifm < 0) {
					...
					O1_strm.write(ifm); O2_strm.wirte(ifm);
				}
			}
		}
	}
...
#pragma HLS ARRAY_PARTITION variable=ofm complete dim=0
	...
	HW_conv2d_8_x_loop: for (x = 0; x < 32; x++) {
		HW_conv2d_8_y_loop: for (y = 0; y < 32; y++) {
...
#pragma HLS ARRAY_PARTITION variable=I complete dim=3
	...
	HW_conv2d_10_x_loop: for (x = 0; x<16 + 1; x++) {
		HW_conv2d_10_y_loop: for (y = 0; y<16 + 1 ; y++) {
...
#pragma HLS ARRAY_PARTITION variable=ofm complete dim=0
	...
	HW_conv2d_13_x_loop: for (x = 0; x < 16; x++) {
		HW_conv2d_13_y_loop: for (y = 0; y < 16; y++) {
...
#pragma HLS INLINE
	...
	int num = 0;
	DATA_T ifm;
	HW_activation_13_x_loop: for (x = 0; x < 4; x++) {
		HW_activation_13_y_loop: for (y = 0; y < 4; y++) {
...
#pragma HLS ARRAY_PARTITION variable=I complete dim=3
	...
	HW_conv2d_17_x_loop: for (x = 0; x<4 + 1; x++) {
		HW_conv2d_17_y_loop: for (y = 0; y<4 + 1 ; y++) {
...
#pragma HLS ARRAY_PARTITION variable=ofm complete dim=0
	...
	HW_conv2d_18_x_loop: for (x = 0; x < 8; x++) {
		HW_conv2d_18_y_loop: for (y = 0; y < 8; y++) {
...
#pragma HLS INLINE
	...
	int num = 0;
	DATA_T ifm;
	HW_activation_14_x_loop: for (x = 0; x < 4; x++) {
		HW_activation_14_y_loop: for (y = 0; y < 4; y++) {
...
#pragma HLS INLINE
	...
	DATA_T ifm1;
	DATA_T ifm2;
	HW_add_7_x_loop_1: for (x = 0; x < 4; x++) {
	    HW_add_7_y_loop_1: for (y = 0; y < 4; y++) {
...
#pragma HLS ARRAY_PARTITION variable=I complete dim=3
	...
	HW_conv2d_19_x_loop: for (x = 0; x<4 + 1; x++) {
		HW_conv2d_19_y_loop: for (y = 0; y<4 + 1 ; y++) {
...
void SW_conv2d_6(DATA_T I[64][32][32], DATA_T O[128][16][16], DATA_T W[128][64][3][3], DATA_T B[128]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (2 *(16 - 1) - 32 + 3)/2;
	for (m = 0; m<128; m++) {
...
void SW_activation_5(DATA_T I[128][16][16], DATA_T O[128][16][16]) {
	int m, x, y, i, j, k;
	for (m = 0; m<128; m++) {
		for (x = 0; x<16; x++) {
			for (y = 0; y<16; y++) {
...
void SW_conv2d_7(DATA_T I[128][16][16], DATA_T O[128][16][16], DATA_T W[128][128][3][3], DATA_T B[128]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(16 - 1) - 16 + 3)/2;
	for (m = 0; m<128; m++) {
...
void SW_conv2d_8(DATA_T I[64][32][32], DATA_T O[128][16][16], DATA_T W[128][64][1][1], DATA_T B[128]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
	for (m = 0; m<128; m++) {
		for (x = 0; x<16; x++) {
...
void SW_activation_6(DATA_T I[128][16][16], DATA_T O[128][16][16]) {
	int m, x, y, i, j, k;
	for (m = 0; m<128; m++) {
		for (x = 0; x<16; x++) {
			for (y = 0; y<16; y++) {
...
void SW_add_3(DATA_T I1[128][16][16], DATA_T I2[128][16][16], DATA_T O[128][16][16]) {
	int m, x, y;
	for (m = 0; m< 128; m++) {
		for (x = 0; x < 16; x++) {
			for(y = 0; y < 16; y++) {
...
void SW_conv2d_10(DATA_T I[128][16][16], DATA_T O[128][16][16], DATA_T W[128][128][3][3], DATA_T B[128]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(16 - 1) - 16 + 3)/2;
	for (m = 0; m<128; m++) {
...
void SW_conv2d_11(DATA_T I[128][16][16], DATA_T O[256][8][8], DATA_T W[256][128][3][3], DATA_T B[256]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (2 *(8 - 1) - 16 + 3)/2;
	for (m = 0; m<256; m++) {
...
void SW_activation_9(DATA_T I[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y, i, j, k;
	for (m = 0; m<256; m++) {
		for (x = 0; x<8; x++) {
			for (y = 0; y<8; y++) {
...
void SW_conv2d_12(DATA_T I[256][8][8], DATA_T O[256][8][8], DATA_T W[256][256][3][3], DATA_T B[256]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(8 - 1) - 8 + 3)/2;
	for (m = 0; m<256; m++) {
...
void SW_conv2d_13(DATA_T I[128][16][16], DATA_T O[256][8][8], DATA_T W[256][128][1][1], DATA_T B[256]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
	for (m = 0; m<256; m++) {
		for (x = 0; x<8; x++) {
...
void SW_activation_10(DATA_T I[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y, i, j, k;
	for (m = 0; m<256; m++) {
		for (x = 0; x<8; x++) {
			for (y = 0; y<8; y++) {
...
void SW_add_5(DATA_T I1[256][8][8], DATA_T I2[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y;
	for (m = 0; m< 256; m++) {
		for (x = 0; x < 8; x++) {
			for(y = 0; y < 8; y++) {
...
void SW_conv2d_14(DATA_T I[256][8][8], DATA_T O[256][8][8], DATA_T W[256][256][3][3], DATA_T B[256]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(8 - 1) - 8 + 3)/2;
	for (m = 0; m<256; m++) {
...
void SW_activation_11(DATA_T I[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y, i, j, k;
	for (m = 0; m<256; m++) {
		for (x = 0; x<8; x++) {
			for (y = 0; y<8; y++) {
...
void SW_conv2d_15(DATA_T I[256][8][8], DATA_T O[256][8][8], DATA_T W[256][256][3][3], DATA_T B[256]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(8 - 1) - 8 + 3)/2;
	for (m = 0; m<256; m++) {
...
void SW_activation_12(DATA_T I[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y, i, j, k;
	for (m = 0; m<256; m++) {
		for (x = 0; x<8; x++) {
			for (y = 0; y<8; y++) {
...
void SW_add_6(DATA_T I1[256][8][8], DATA_T I2[256][8][8], DATA_T O[256][8][8]) {
	int m, x, y;
	for (m = 0; m< 256; m++) {
		for (x = 0; x < 8; x++) {
			for(y = 0; y < 8; y++) {
...
void SW_conv2d_16(DATA_T I[256][8][8], DATA_T O[512][4][4], DATA_T W[512][256][3][3], DATA_T B[512]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (2 *(4 - 1) - 8 + 3)/2;
	for (m = 0; m<512; m++) {
...
void SW_activation_13(DATA_T I[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y, i, j, k;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
			for (y = 0; y<4; y++) {
...
void SW_conv2d_17(DATA_T I[512][4][4], DATA_T O[512][4][4], DATA_T W[512][512][3][3], DATA_T B[512]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(4 - 1) - 4 + 3)/2;
	for (m = 0; m<512; m++) {
...
void SW_conv2d_18(DATA_T I[256][8][8], DATA_T O[512][4][4], DATA_T W[512][256][1][1], DATA_T B[512]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
...
void SW_activation_14(DATA_T I[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y, i, j, k;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
			for (y = 0; y<4; y++) {
...
void SW_add_7(DATA_T I1[512][4][4], DATA_T I2[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y;
	for (m = 0; m< 512; m++) {
		for (x = 0; x < 4; x++) {
			for(y = 0; y < 4; y++) {
...
void SW_conv2d_19(DATA_T I[512][4][4], DATA_T O[512][4][4], DATA_T W[512][512][3][3], DATA_T B[512]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(4 - 1) - 4 + 3)/2;
	for (m = 0; m<512; m++) {
...
void SW_activation_15(DATA_T I[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y, i, j, k;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
			for (y = 0; y<4; y++) {
...
void SW_conv2d_20(DATA_T I[512][4][4], DATA_T O[512][4][4], DATA_T W[512][512][3][3], DATA_T B[512]) {
	int m, x, y, i, j, k;
	DATA_T ifm, ofm;
    int p = (1 *(4 - 1) - 4 + 3)/2;
	for (m = 0; m<512; m++) {
...
void SW_activation_16(DATA_T I[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y, i, j, k;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
			for (y = 0; y<4; y++) {
...
void SW_add_8(DATA_T I1[512][4][4], DATA_T I2[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y;
	for (m = 0; m< 512; m++) {
		for (x = 0; x < 4; x++) {
			for(y = 0; y < 4; y++) {
...
void SW_activation_17(DATA_T I[512][4][4], DATA_T O[512][4][4]) {
	int m, x, y, i, j, k;
	for (m = 0; m<512; m++) {
		for (x = 0; x<4; x++) {
			for (y = 0; y<4; y++) {
...
W48_i_m_loop: for (m=0; m<200; m++) {
  W48_i_k_loop: for (k=0; k<512; k++) {
  W48_i[m][k] = W48[m][k];
      }
}
B48_i_m_loop: for (m=0; m<200; m++) {
	B48_i[m] = B48[m];
}
...
void resnet18_sw(DATA_T I[3][64][64],DATA_T W1[64][3][3][3],DATA_T B1[64], DATA_T W3[64][64][3][3],DATA_T B3[64], DATA_T W4[64][64][3][3],DATA_T B4[64], DATA_T W5[64][64][1][1],DATA_T B5[64], DATA_T W8[64][64][3][3],DATA_T B8[64], DATA_T W10[64][64][3][3],DATA_T B10[64], DATA_T W13[128][64][3][3],DATA_T B13[128], DATA_T W15[128][128][3][3],DATA_T B15[128], DATA_T W16[128][64][1][1],DATA_T B16[128], DATA_T W19[128][128][3][3],DATA_T B19[128], DATA_T W21[128][128][3][3],DATA_T B21[128], DATA_T W24[256][128][3][3],DATA_T B24[256], DATA_T W26[256][256][3][3],DATA_T B26[256], DATA_T W27[256][128][1][1],DATA_T B27[256], DATA_T W30[256][256][3][3],DATA_T B30[256], DATA_T W32[256][256][3][3],DATA_T B32[256], DATA_T W35[512][256][3][3],DATA_T B35[512], DATA_T W37[512][512][3][3],DATA_T B37[512], DATA_T W38[512][256][1][1],DATA_T B38[512], DATA_T W41[512][512][3][3],DATA_T B41[512], DATA_T W43[512][512][3][3],DATA_T B43[512], DATA_T W48[200][512],DATA_T B48[200],  DATA_T O48_SW[200]) {
	...
	static DATA_T O13_SW[128][16][16];
	static DATA_T O14_SW[128][16][16];
	static DATA_T O15_SW[128][16][16];
	static DATA_T O16_SW[128][16][16];
	static DATA_T O17_SW[128][16][16];
	...
	static DATA_T O24_SW[256][8][8];
	static DATA_T O25_SW[256][8][8];
	static DATA_T O26_SW[256][8][8];
	static DATA_T O27_SW[256][8][8];
	static DATA_T O28_SW[256][8][8];
	static DATA_T O29_SW[256][8][8];
	static DATA_T O30_SW[256][8][8];
	static DATA_T O31_SW[256][8][8];
	static DATA_T O32_SW[256][8][8];
	static DATA_T O33_SW[256][8][8];
	static DATA_T O34_SW[256][8][8];
	static DATA_T O35_SW[512][4][4];
	static DATA_T O36_SW[512][4][4];
	static DATA_T O37_SW[512][4][4];
	static DATA_T O38_SW[512][4][4];
	static DATA_T O39_SW[512][4][4];
	static DATA_T O40_SW[512][4][4];
	static DATA_T O41_SW[512][4][4];
	static DATA_T O42_SW[512][4][4];
	static DATA_T O43_SW[512][4][4];
	static DATA_T O44_SW[512][4][4];
	static DATA_T O45_SW[512][4][4];
	static DATA_T O46_SW[512][4][4];
