...
class C_verifier(CodeGenerators):
        ...
        for layer in self.model_sw.layers :
            ...
            if layer_type == 'Conv2D':
                ...
                if len(self.model_sw.graphs[layer.config['name']]['in']) == 3 :
                    ...
                    self.format = 'SW_{}(O{}_SW, O{}_SW, O{}_SW,O{}_SW);\n\t'.format(layer.config['name'], inp1, inp2,
                                                                                     inp3, l_n)
                    sw_call_layer += self.format
    ...
    def generate(self):
        file = open('C_verifier_code/googlenet/C_verifier.cpp', 'w')
...
int main(int argc, char *argv[]){{
    ...
    FILE *o_stream = fopen("../../cpp_generator/googlenet/Output/C_output.txt", "w");
    if (o_stream == NULL) printf("Output file was not opened");
    FILE *c_num = fopen("../../cpp_generator/googlenet/Output/c_output_num.txt", "w");
