...
class Conv2D(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post='') :
        ...
        # code
        if use_bias:
            conv_s = open("cpp_generator/vgg16/Template/Function/Conv2D_same_stride_bias.txt")
            conv_v = open("cpp_generator/vgg16/Template/Function/Conv2D_valid_bias.txt")
            conv2d_same = conv_s.read()
            conv2d_valid = conv_v.read()
        else:
            conv_s = open("cpp_generator/vgg16/Template/Function/Conv2D_same_stride.txt")
            conv_v = open("cpp_generator/vgg16/Template/Function/Conv2D_valid.txt")
            conv2d_same = conv_s.read()
            conv2d_valid = conv_v.read()
