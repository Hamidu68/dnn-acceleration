...
class MaxPooling2D(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post=''):
        ...
        # code
        mxp = open("cpp_generator/vgg16/Template/Function/MaxPooling2D.txt")
        template = mxp.read()
...
class Flatten(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post=''):
        ...
        # code
        fla = open("cpp_generator/vgg16/Template/Function/Flatten.txt")
        template = fla.read()
        func = template.format(Name=self.config["name"], Input_channel=input_shape[3], Input_width=input_shape[1],
...
class Dense(Layers):
    ...
    def __init__(self, config={}, inputs=[], dtype='DATA_T', layer_odr=0, post=''):
        ...
        # code
        if use_bias:
            den_s = open("cpp_generator/vgg16/Template/Function/Dense_Softmax_bias.txt")
            den_r = open("cpp_generator/vgg16/Template/Function/Dense_Relu_bias.txt")
            dense_softmax = den_s.read()
            dense_relu = den_r.read()
        else:
            den_s = open("cpp_generator/vgg16/Template/Function/Dense_Softmax.txt")
            den_r = open("cpp_generator/vgg16/Template/Function/Dense_Relu.txt")
            dense_softmax = den_s.read()
            dense_relu = den_r.read()
